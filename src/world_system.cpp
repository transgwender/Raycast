...
bool WorldSystem::step(float elapsed_ms_since_last_update) {
    ...
    for (int i = (int)motions_registry.components.size() - 1; i >= 0; --i) {
        ...
        if (motion.position.x + abs(motion.scale.x) < 0.f) {
            if (!registry.players.has(
                    motions_registry.entities[i])) // don't remove the player
    ...
    if (registry.deadlys.components.size() <= MAX_NUM_EELS &&
        next_eel_spawn < 0.f) {
        // reset timer
        next_eel_spawn = (EEL_SPAWN_DELAY_MS / 2) +
                         uniform_dist(rng) * (EEL_SPAWN_DELAY_MS / 2);

        // create Eel with random initial position
        createEel(
            renderer,
            vec2(50.f + uniform_dist(rng) * (window_width_px - 100.f), 100.f));
    }

    // spawn fish
    next_fish_spawn -= elapsed_ms_since_last_update * current_speed;
    if (registry.eatables.components.size() <= MAX_NUM_FISH &&
        next_fish_spawn < 0.f) {
    ...
    // !!! TODO A1: update LightUp timers and remove if time drops below zero,
    // similar to the death counter
...
void WorldSystem::restart_game() {
    ...
    // create a new Salmon
    player_salmon =
        createSalmon(renderer, {window_width_px / 2, window_height_px - 200});
    ...
    for (uint i = 0; i < 20; i++) {
        ...
        Entity egg = createEgg({ uniform_dist(rng) * w, h - uniform_dist(rng) *
    20 }, { radius, radius }); float brightness = uniform_dist(rng) * 0.5 + 0.5;
        ...
        if (registry.players.has(entity)) {
            ...
            if (registry.deadlys.has(entity_other)) {
                ...
                if (!registry.deathTimers.has(entity)) {
                    ...
                    // !!! TODO A1: change the salmon's orientation and color on
                    // death
                }
            }
            // Checking Player - Eatable collisions
                ...
                if (!registry.deathTimers.has(entity)) {
                    ...
                    // !!! TODO A1: create a new struct called LightUp in
                    // components.hpp and add an instance to the salmon entity
                    // by modifying the ECS registry
                }
            }
        }
    }
