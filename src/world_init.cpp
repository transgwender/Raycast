...
Entity createEel(RenderSystem* renderer, vec2 position) {
    ...
    registry.renderRequests.insert(entity, {TEXTURE_ASSET_ID::EEL,
                                            EFFECT_ASSET_ID::TEXTURED,
                                            GEOMETRY_BUFFER_ID::SPRITE});
...
Entity createLine(vec2 position, vec2 scale) {
    ...
    // Store a reference to the potentially re-used mesh object (the value is
    // stored in the resource cache)
    registry.renderRequests.insert(entity, {TEXTURE_ASSET_ID::TEXTURE_COUNT,
                                            EFFECT_ASSET_ID::EGG,
...
Entity createEgg(vec2 pos, vec2 size) {
    ...
    registry.deadlys.emplace(entity);
    registry.renderRequests.insert(
        entity, {TEXTURE_ASSET_ID::TEXTURE_COUNT, // TEXTURE_COUNT indicates
                                                  // that no txture is needed
                 EFFECT_ASSET_ID::EGG, GEOMETRY_BUFFER_ID::EGG});
