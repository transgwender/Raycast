...
bool RenderSystem::init(GLFWwindow* window_arg) {
    ...
    // Hint: Ask your TA for how to set up pretty OpenGL error callbacks.
    // This can not be done in macOS, so do not enable
    // it unless you are on Linux or Windows. You will need to change the window
    // creation code to use OpenGL 4.3 (not suported in macOS) and add
    // additional .h and .cpp glDebugMessageCallback((GLDEBUGPROC)errorCallback,
    // nullptr);

    // We are not really using VAO's but without at least one bound we will
...
void RenderSystem::initializeGlTextures() {
    glGenTextures((GLsizei)texture_gl_handles.size(),
...
void RenderSystem::initializeGlEffects() {
    for (uint i = 0; i < effect_paths.size(); i++) {
        ...
        bool is_valid = loadEffectFromFile(vertex_shader_name,
                                           fragment_shader_name, effects[i]);
...
bool RenderSystem::initScreenTexture() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_width,
                 framebuffer_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    gl_has_errors();
