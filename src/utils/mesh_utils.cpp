...
bool MeshUtils::loadFromOBJFile(const std::string& obj_path, std::vector<ColoredVertex>& out_vertices,
...
#pragma warning(disable : 4996)
    ...
    if (file == NULL) {
        std::cerr << "Impossible to open the file ! Are you in the right path ? See Tutorial 1 for details"
                  << std::endl;
        getchar();
        return false;
    }
    ...
    while (true) {
        ...
        if (strcmp(lineHeader, "v") == 0) {
            ...
            {
                // Note first vertex index is already consumed by the first fscanf call (match ==1) since it aborts on
                // the first error
